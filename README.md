...
```
<VirtualHost *>
    DocumentRoot "/path/to/MonkeyTest/build/public/"
    ServerName MonkeyTest.dev
    <Directory /path/to/MonkeyTest/build/public/>
        AllowOverride All
        Order allow,deny
        Allow from all
    </Directory>
...
To avoid any extra configuration, create a database called `MonkeyTest` and a user called
`MonkeyTest` with password `MonkeyTest` with `SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER`
permissions on the `MonkeyTest` database, with access from localhost.
...
Visit [http://MonkeyTest.dev/](http://MonkeyTest.dev/) and you should see the home page. Check your browser's error console
for any load errors or javascript errors.

Visit [http://MonkeyTest.dev/tests/](http://MonkeyTest.dev/tests/) to run the automated acceptance tests.
